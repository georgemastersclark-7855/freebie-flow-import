  ...
  return (
    <div className="mx-auto max-w-[1500px] px-4 py-8 sm:px-7 lg:px-10 lg:py-10 xl:px-14">
      ...
      <section className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[{ label: "Songs uploaded", value: `${submitted}/${total}`, detail: `${Math.max(0, total - submitted)} still missing`, icon: Headphones }, { label: "Stems ready", value: `${stemsReady}/${total}`, detail: "Surgery eligible", icon: Sparkles }, { label: "Active students", value: total.toString(), detail: "Current cohort", icon: Users }, { label: "Needs a check-in", value: rescue.toString(), detail: "Personal rescue queue", icon: AlertTriangle }].map(({ label, value, detail, icon: Icon }) => (
          <div key={label} className="mp-card rounded-2xl p-5"><div className="flex items-center justify-between"><div className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#77766f]">{label}</div><Icon size={17} className="text-[#77766f]" /></div><div className="mt-3 text-3xl font-black tracking-[-0.06em] text-white">{value}</div><div className="mt-1 text-xs text-[#77766f]">{detail}</div></div>
        ))}
      </section>
        ...
        <div className="mp-card overflow-hidden rounded-3xl">
          ...
          <div className="mp-scrollbar overflow-x-auto">
            <table className="w-full min-w-[880px] border-collapse text-left">
              <thead><tr className="border-b border-white/[0.08] text-[10px] font-bold uppercase tracking-[0.12em] text-[#66655f]"><th className="px-6 py-3.5">Student</th><th className="px-3 py-3.5">Loops</th><th className="px-3 py-3.5">Song</th><th className="px-3 py-3.5">Stems</th><th className="px-3 py-3.5">Calls</th><th className="px-3 py-3.5">Surgery</th><th className="px-3 py-3.5">Feedback</th><th className="px-6 py-3.5 text-right">Status</th></tr></thead>
              <tbody className="divide-y divide-white/[0.065]">
                {visibleStudents.map((student) => (
                  <tr key={student.id} className="group transition hover:bg-white/[0.02]"><td className="px-6 py-4"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-white/[0.06] text-[11px] font-black text-[#d2cfc6]">{student.initials}</span><div><div className="text-sm font-bold text-[#e5e1d8]">{student.name}</div><div className="text-[11px] text-[#66655f]">{student.lastActivity}</div></div></div></td><td className="px-3 py-4"><span className="text-sm font-black text-[#d9d6cd]">{student.ideasRequired ? `${student.ideasSubmitted}/${student.ideasRequired}` : "None required"}</span></td><td className="px-3 py-4">{student.songSubmitted ? <Check size={16} className="text-[#aaa99f]" /> : <span className="text-[#4f4e49]">–</span>}</td><td className="px-3 py-4">{student.stemsSubmitted ? <Check size={16} className="text-[#aaa99f]" /> : <span className="text-[#4f4e49]">–</span>}</td><td className="px-3 py-4 text-xs font-semibold text-[#aaa99f]">{student.attendanceTotal ? `${student.attendance}/${student.attendanceTotal}` : "–"}</td><td className="px-3 py-4 text-xs font-semibold text-[#aaa99f]">{student.surgeryCount}</td><td className="px-3 py-4"><span className="rounded-full border border-white/10 bg-white/[0.035] px-2 py-1 text-[10px] font-bold text-[#aaa99f]">{feedbackLabels[student.feedbackState]}</span></td><td className="px-6 py-4 text-right"><span className={cx("inline-flex items-center gap-2 text-[10px] font-bold", student.status === "on_track" ? "text-[#77766f]" : "text-[#d7bd65]")}><span className="h-1.5 w-1.5 rounded-full bg-current" />{student.status === "on_track" ? "On track" : student.status === "needs_attention" ? "Check in" : "Not started"}</span></td></tr>
                ))}
              </tbody>
            </table>
          </div>
