...
const JulyGiveaway = () => {
  ...
  const fetchLeadMagnets = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('lead_magnets').select('*').eq('is_active', true).eq('title', 'Rob Late IG Stems Giveaway - July 2025').order('created_at', {
        ascending: false
      });
      if (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Submitting form for IG Stems Giveaway with:', {
      name,
      email,
      utmParams
    });
  ...
  const handleDownload = async (leadMagnet: LeadMagnet) => {
    ...
    try {
      ...
      const {
        error: downloadError
      } = await supabase.from('lead_downloads').insert(downloadData);
      if (downloadError) {
      ...
      if (updateError) {
        ...
        // Update local state
        setLeadMagnets(prev => prev.map(lm => lm.id === leadMagnet.id ? {
          ...lm,
          download_count: lm.download_count + 1
        } : lm));
      }
      toast({
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-base text-gray-300 max-w-2xl mx-auto mb-8 font-zurich-condensed">Get exclusive mix-ready stems from Rob Late's latest production vault. Perfect for reverse-engineering and music producers who need some added flavour for their beats.</p>
          </div>

          {/* Form or Download Section */}
          {!isSubmitted ? <div>
              <Card className="max-w-md mx-auto bg-gray-950/90 backdrop-blur-sm border-gray-700">
                <CardContent className="p-8">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      <Input type="text" placeholder="Your Name" value={name} onChange={e => setName(e.target.value)} required className="bg-gray-800 border-gray-600 text-white placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    
                    <div>
                      <Input type="email" placeholder="Your Email" value={email} onChange={e => setEmail(e.target.value)} required className="bg-gray-800 border-gray-600 text-white placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    
                    <Button type="submit" className="w-full bg-gradient-to-r from-[#DEFF00] to-[#B8CC00] hover:from-[#B8CC00] hover:to-[#9BAA00] text-black font-semibold font-zurich-condensed" disabled={isSubmitting}>
              ...
              <div className="relative max-w-md mx-auto mt-12">
                <img src="/lovable-uploads/c3977c47-ba35-48ce-ab95-c20b978cf4f8.png" alt="Rob Late in his studio - July Giveaway" className="w-full rounded-2xl shadow-2xl border border-gray-700" style={{
              ...
              <div className="grid gap-6">
                {leadMagnets.map(leadMagnet => <Card key={leadMagnet.id} className="bg-gray-950/90 backdrop-blur-sm border-gray-700 hover:border-blue-500/50 transition-colors">
                    <CardContent className="p-6">
                      ...
                      <Button onClick={() => handleDownload(leadMagnet)} className="w-full bg-gradient-to-r from-[#DEFF00] to-[#B8CC00] hover:from-[#B8CC00] hover:to-[#9BAA00] text-black font-semibold font-zurich-condensed" size="lg">
              ...
              {leadMagnets.length === 0 && <div className="text-center py-12">
