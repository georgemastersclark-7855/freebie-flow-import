    ...
    <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-7 lg:px-10 lg:py-10 xl:px-12">
      ...
      <section className="overflow-hidden rounded-[28px] border border-white/10 bg-[#151512]">
        ...
        <div className="flex flex-col gap-4 border-t border-white/[0.08] bg-white/[0.025] p-5 sm:flex-row sm:items-center sm:justify-between sm:px-9">
          ...
          {allComplete ? <Link to="/mentorship-portal/submissions" className="mp-focus-ring inline-flex items-center justify-center gap-2 rounded-xl bg-[#D3FF02] px-5 py-3 text-sm font-bold text-black">Go to my submissions <ArrowRight size={16} /></Link> : nextIsBooking ? <button type="button" onClick={openBooking} className="mp-focus-ring inline-flex items-center justify-center gap-2 rounded-xl bg-[#D3FF02] px-5 py-3 text-sm font-bold text-black">Book my onboarding <ArrowRight size={16} /></button> : nextTask ? <a href={`#onboarding-${nextTask.id}`} className="mp-focus-ring inline-flex items-center justify-center gap-2 rounded-xl bg-[#D3FF02] px-5 py-3 text-sm font-bold text-black">{nextTask.actionLabel} <ArrowRight size={16} /></a> : <a href="#onboarding-steps" className="mp-focus-ring inline-flex items-center gap-2 text-sm font-semibold text-[#f2efe6]">Explore your next steps <ArrowRight size={16} /></a>}
        </div>
      </section>
      ...
      <div className="mt-8 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_280px]">
        <section id="onboarding-steps" className="min-w-0 scroll-mt-24">
          ...
          <Link to="/mentorship-portal/submissions" className="mp-focus-ring mt-6 inline-flex items-center gap-2 rounded-lg py-2 text-sm font-semibold text-[#b6b3a8]">Take a look at my submissions <ArrowRight size={15} /></Link>
        </section>
